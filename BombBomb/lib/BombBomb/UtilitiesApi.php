...
class UtilitiesApi
{
     ...
     * @param string $name The name of the OAuth client. e.g. MyCrm DEV, or MyCrm PROD (required)
     * @param string $redirectUri The URI to direct the client to after logging in. (required)
     * @return Array of \BombBomb\OAuthClient, HTTP status code, HTTP response headers (array of strings)
     * @throws \Swagger\Client\ApiException on non-2xx response
     */
    public function createOAuthClientWithHttpInfo($name, $redirectUri)
    {
        ...
        try {
            ...
            return array($this->apiClient->getSerializer()->deserialize($response, '\BombBomb\OAuthClient', $httpHeader), $statusCode, $httpHeader);
        } catch (ApiException $e) {
            switch ($e->getCode()) {
                case 200:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\BombBomb\OAuthClient', $e->getResponseHeaders());
